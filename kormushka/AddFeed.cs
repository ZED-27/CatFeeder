...
namespace kormushka
{
    public partial class AddFeed : Form
    {
        ...
        private void buttonRegistration_Click(object sender, EventArgs e)
        {
            ...
            DB db = new DB();
            MySqlCommand command = new MySqlCommand("INSERT INTO `feeder` ( `login`, `condition`, `name`, `feed`) VALUES (@login, @condition, @name, @feed)", db.getConnection());

            command.Parameters.Add("@login", MySqlDbType.VarChar).Value = loginField.Text;
            command.Parameters.Add("@condition", MySqlDbType.VarChar).Value = condition.Text;
            command.Parameters.Add("@name", MySqlDbType.VarChar).Value = nameFeed.Text;
        ...
        public bool isloginExist()
        {
            ...
            MySqlCommand command = new MySqlCommand("SELECT * FROM `trueusers` WHERE `login`=@login", db.getConnection());
            command.Parameters.Add("@login", MySqlDbType.VarChar).Value = loginField.Text;
